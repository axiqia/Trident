...
Trident is currently validated to work with Intel Haswell-EP based Xeon processors. The experiments are run on E5-2630 v3 processor based system with dual sockets and eight cores each. The hyper threading is disabled for all the evaluations and turbo boost is left at its default setting similar to a production server. The system also has high speed PCIe SSDs for storage and 64GB DDR4 ECC RAM
